...
def test_validate_stock_data_nominal():
    ...
    THEN check that the validation is sucessful
    """
    stock_data = StockModel(
        stock_symbol="SBUX", number_of_shares="100", purchase_price="45.67"
    )
...
def test_validate_stock_data_invalid_stock_symbol():
    ...
    with pytest.raises(ValueError):
        StockModel(
            stock_symbol="SBUX123", number_of_shares="100", purchase_price="45.67"
        )
...
def test_validate_stock_data_invalid_number_of_shares():
    ...
    with pytest.raises(ValidationError):
        StockModel(
            stock_symbol="SBUX", number_of_shares="100.1221", purchase_price="45.67"
        )
...
def test_validate_stock_data_invalid_purchase_price():
    ...
    """
    with pytest.raises(ValidationError):
        StockModel(stock_symbol="SBUX", number_of_shares="100", purchase_price="45,67")
...
def test_validate_stock_data_missing_purchase_price():
    ...
    """
    with pytest.raises(ValidationError):
        StockModel(stock_symbol="SBUX", number_of_shares="100")
