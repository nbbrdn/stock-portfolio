...
def test_about_page():
    ...
    with app.test_client() as client:
        ...
        assert (
            b"This application is built using the Flask web framework" in response.data
        )
...
def test_get_add_stock_page():
    ...
    """
    with app.test_client() as client:
        response = client.get("/add-stock")
        assert response.status_code == 200
        assert b"Stock Portfolio App" in response.data
        assert b"Add a Stock" in response.data
        assert (
            b"Stock Symbol <em>(required, 1-5 uppercase letters)</em>" in response.data
        )
